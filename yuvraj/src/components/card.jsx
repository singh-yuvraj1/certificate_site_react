import React from 'react'

const card = () => {
  return (
   <div className='w-58 h-62 rounded-3xl bg-white shadow-lg m-16 p-4'>
       <div className='flex justify-between items-start'>
          
        <img className =" w-12 h-12 object-cover rounded-full"  src="data:image/png;base64," alt="" srcset="" />
            <div className='flex flex-col items-center'>      
              <Bookmark size = '33'/>
              <span className='text-sm'>Save</span>    
            </div>  
       </div>

       <div className='mt-3'>
          <h2 className='text-lg font-sans' >Amazon <span className='text-sm ml- text-gray-500'>5 days ago</span> </h2>
          <h1>Senior UI/UX Designer</h1>
          <div className='flex gap-3 mt-2'>
             <button className='bg-gray-400 rounded-3xl text-sm w-23 h-7' >Part-Time</button>
             <button className='bg-gray-400 rounded-3xl text-sm w-23 h-7'>Senior-level</button>
           </div>  
        </div>
         <hr className='mt-8'></hr>
       <div className='mt-2 flex gap-13' >
        
          <h3>$120/hr</h3> 
          <button className= "bg-gray-400 rounded-3xl text-sm w-23">Apply Now</button>
        </div>
    </div>
  )
}

export default card
    